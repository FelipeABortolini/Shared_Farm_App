...
import 'package:flv_farm/pages/culture/utils/update_to_recipe.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';
import 'package:rflutter_alert/rflutter_alert.dart';

import '../../../providers/providers.dart';
import '../../../shared/alert.dart';
import 'culture_products_info_page.dart';
import 'phase_option.dart';
import 'reset_all_prods.dart';

// ignore: must_be_immutable
class SelectCulturePhasesPage extends HookConsumerWidget {
  SelectCulturePhasesPage({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void resetControlls() {
      ...
      recipeOption.setAutomatic();
    }

    return Scaffold(
      appBar: AppBar(
        title: const Text('Fase da Cultura'),
        leading: IconButton(
          onPressed: () {
            resetControlls();
              ...
              children: [
                SizedBox(
                  ...
                  child: Column(
                    children: [
                      const Text(
                        'Deseja preencher automaticamente os produtos da receita?',
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          const Text('Não'),
                          Checkbox(
                            value: recipeOption.no,
                            onChanged: (valueNot) {
                              recipeOption.resetAutomatic();
                            },
                          ),
                        ],
                      ),
                            ...
                            children: [
                              ...
                              (phaseOption.phase == 'APLICAÇÃO R1/R3')
                                  ...
                                  : Opacity(
                                      opacity: 0.35,
                                      child: PhaseOption(
                                          phase: 'APLICAÇÃO R1/R3')),
                              (phaseOption.phase == 'APLICAÇÃO R5/R5.2')
                                  ? Opacity(
                                      opacity: 1,
                                      child: PhaseOption(
                                          phase: 'APLICAÇÃO R5/R5.2'))
                                  : Opacity(
                                      opacity: 0.35,
                                      child: PhaseOption(
                                          phase: 'APLICAÇÃO R5/R5.2')),
                              // (cultureStage.stage != 'start')
                              // ?
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          if (phaseOption.phase != '') {
            alreadyExists = false;
            if (cultureStage.stage == "finish" ||
                cultureStage.stage == "continue") {
              for (var culture in userCultures.culturesList) {
                if (culture["cultureName"] ==
                        userCultures.culturesList[indexCultureSelected.index!]
                            ['cultureName'] &&
                    phaseSelected.phase ==
                        userCultures.culturesList[indexCultureSelected.index!]
                            ['phase']) {
            ...
            // if (!alreadyExists || (phaseOption.phase == "COLHEITA")) {
            resetAllProds(
              prod1,
              prod2,
              prod3,
            ...
            //   await alert.alert(
            //       context,
            //       "Essa fase já foi registrada nessa cultura.",
            //       "ERRO",
            //       AlertType.error);
            // }
          } else {
            ...
            await alert.alert(
                context,
                "Selecione uma fase de cultura para avançar!",
                "ERRO",
                AlertType.error);
          }
        },
